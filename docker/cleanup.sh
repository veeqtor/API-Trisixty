...
clean() {
    ...
    API_IMAGE=$(docker images | grep "rest_api" | awk '{print $3}' )
    CONTAINERS=$(docker ps -a -q)
    VOLUME=$(docker volume ls | grep "api-trisixty_tmp-docker_db" | awk '{print $2}')
    ...
    if [[ ! -z $API_IMAGE ]]; then
        ...
        docker rmi api-trisixty_rest_api:latest
    fi
    ...
    if [[ -z $VOLUME ]]; then
        ...
        docker volume rm api-trisixty_tmp-docker_db
    fi
}
